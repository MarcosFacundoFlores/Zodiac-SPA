  ...
  <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6" role="list">
    ...
    >
      <div class="p-6 text-center text-white">
        <div class="text-6xl mb-4 group-hover:scale-110 transition-transform duration-300" aria-hidden="true">
          {{ sign.emoji }}
        </div>
        <h3 class="text-xl font-bold mb-2">{{ sign.name }}</h3>
        <p class="text-sm opacity-80 mb-3">{{ sign.dates }}</p>
        <div
          :class="`inline-block px-3 py-1 rounded-full bg-gradient-to-r ${getElementColor(sign.element)} text-xs font-semibold`"
        >
          {{ sign.element }}
        </div>
      </div>
